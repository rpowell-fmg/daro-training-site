...
# Labels {background-gradient="linear-gradient(to bottom right, #FFFFFF, #7CDCFF, #7CDCFF, #7CDCFF)"}
```{r}
#| label: packages
#| echo: FALSE
#| message: FALSE
...
# Variable names and labels {background-gradient="linear-gradient(to bottom right, #FFFFFF, #FECCDD, #FECCDD, #FECCDD)"}

# Rename variables {background-gradient="linear-gradient(to bottom right, #FFFFFF, #7CDCFF, #7CDCFF, #7CDCFF)"}
...
`rename()` function from **dplyr** package allows us to update variable names one at a time in the format `new_var` = `old_var`
...
# Relabel variables {background-gradient="linear-gradient(to bottom right, #FFFFFF, #FECCDD, #FECCDD, #FECCDD)"}
...
# Variable values and labels {background-gradient="linear-gradient(to bottom right, #FFFFFF, #7CDCFF, #7CDCFF, #7CDCFF)"}

## See and set value labels {.smaller}

```{r}
#| label: create-dup-data
#| echo: false
...
`val_labels()` allows us to see or set all value labels for a particular variable

::: column
```{r}
#| label: val-labels
val_labels(spss_ex$Q2)
```
:::
...
::: fragment
```{r}
#| label: val-labels-set
val_labels(spss_ex$Q2) <- c(
    "No, None" = 0,
    "Yes, Previously" = 1,
    "Yes, Currently" = 2,
    "Yes, College" = 3,
...
## Edit specific value label {.smaller}

`val_label()` (not plural) allows us to see or set a single label for a single value of a particular variable

::: column
```{r}
#| label: val-label
val_label(spss_ex$Q2, 0)
```
:::
...
## Setting labels in pipelines - single variable
...
`set_value_labels()` allows us to set the labels for the data in our dataset in the format of `c("label1" = value1, "label2" = value2, ...)`
...
```{r}
#| label: val-labels-2out
val_labels(spss_Q2new$Q2)
```


## Setting labels in pipelines - multiple variable {.smaller}

::: {.column}
```{r}
#| label: val-labels-3
val_labels(spss_ex$Q2)
val_labels(spss_ex$Q4)
```
:::
...
```{r}
#| label: val-labels-4
#| code-line-numbers: "|4-10|11-14"
spss_ex_newlabels<-spss_ex %>% 
  zap_labels() %>% 
  set_value_labels(
    Q2 = c(
      "No, None" = 0,
...
::: {.fragment}
```{r}
#| label: vallabels-set2-view
val_labels(spss_ex_newlabels$Q2)
val_labels(spss_ex_newlabels$Q4)
```
:::
::::

## Setting labels in pipelines - All at once
...
We're currently creating a process that will automate the labels creation for all variables at once based on the metadata and AQ.
...
# Data dictionary/codebook {background-gradient="linear-gradient(to bottom right, #FFFFFF, #FECCDD, #FECCDD, #FECCDD)"}
...
`look_for()` creates the data dictionary

`lookfor_to_long_format()` puts it in an easy to view tibble format for us
...
## Data dictionary (cont.) {.smaller}

Notice how the "levels" column doesn't get populated in the long format (may be a bug)

. . .

We know how to handle strings and can pull out the levels/values from the labels
...
spss_ex %>% 
  look_for() %>%
  lookfor_to_long_format() %>% 
  mutate(levels = str_extract(value_labels,"(?<=\\[).*(?=\\])")) %>% 
  rename(values=levels)
```
